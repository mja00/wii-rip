...
// NOTE: Altered source version of Funcs.h from the wii-banner-player project.
// Modifications:
//   - Removed `#include <boost/foreach.hpp>`. The original used BOOST_FOREACH,
//     which the upstream comment already noted should be replaced. The macro
//     `foreach(decl, container)` now expands to a C++11 range-based for loop,
//     dropping the Boost build dependency on every platform.
