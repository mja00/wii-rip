...
// NOTE: Altered source version of WrapGx.cpp from the wii-banner-player project.
// Modifications:
//   - Removed `#include <GL/glu.h>` and `#include <GL/gl.h>`. The file does
//     not call any GLU function, and `#include <GL/glew.h>` already exposes
//     all GL declarations the file uses. <GL/glu.h> is not present in modern
//     macOS SDKs, and <GL/gl.h> lives at <OpenGL/gl.h> on Apple — dropping
//     both stale includes lets the file build cleanly on Apple Silicon.
