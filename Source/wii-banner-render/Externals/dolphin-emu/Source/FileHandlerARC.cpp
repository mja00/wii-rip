...
// Modifications:
//   - Added FindFileMatching() implementation. See FileHandlerARC.h for
//     the rationale.
...
std::string CARCFile::FindFileMatching(const std::string& suffix) const
{
	if (!m_Initialized || suffix.empty())
		return std::string();

	for (const SFileInfo& fi : m_FileInfoVector)
	{
		if (fi.IsDirectory())
			continue;

		const size_t path_len = std::strlen(fi.m_FullPath);
		const size_t suf_len  = suffix.size();
		if (path_len < suf_len)
			continue;
		if (std::memcmp(fi.m_FullPath + (path_len - suf_len),
				suffix.c_str(), suf_len) == 0)
		{
			return std::string(fi.m_FullPath);
		}
	}
	return std::string();
}
...
std::vector<std::string> CARCFile::ListAllFiles() const
{
	...
	{
		if (!fi.IsDirectory())
			out.emplace_back(fi.m_FullPath);
	}
	return out;
}
